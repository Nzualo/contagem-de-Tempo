...
# Local optional .env (safe on Streamlit Cloud too)
try:
...
# Session cache for IA demo (avoid repeated quota consumption)
if "demo_cache" not in st.session_state:
    st.session_state["demo_cache"] = {}

with st.sidebar:
    st.header("Dados-base")
    nome = st.text_input("Nome do funcionario", value="")

    inicio_funcoes = st.date_input("Inicio de funcoes", value=date(2017, 2, 28))
    fim_funcoes = st.date_input("Fim (ultimo dia) de funcoes", value=date.today())
    inicio_desconto = st.date_input(
        "Inicio do desconto (nomeacao provisoria / inicio no sistema)",
        value=date(2017, 6, 30),
    )

    st.divider()
    st.header("Dados para o formulario (Topo)")
    categoria = st.text_input("Categoria", value="")
    classe = st.text_input("Classe", value="")
    escalao = st.text_input("Escalao", value="")
    ...
    remuneracao_ou_pensao = st.number_input(
        "Remuneracao/Pensao para limite 1/3 (Mt)",
        min_value=0.0,
        value=19258.00,
        step=10.0,
    )
    ...
    usar_ia = st.checkbox("Usar IA (Gemini) na demonstracao do PDF", value=True)
    gemini_model = st.text_input("Modelo Gemini (preferido)", value="gemini-2.5-pro")
    st.caption("Secrets: GOOGLE_API_KEY (ou GEMINI_API_KEY).")
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Tempo de servico", "Tempo descontado", "Tempo nao descontado", "Fixacao de encargos"]
)

with tab1:
    st.subheader("Tempo de servico")
    st.info(f"Periodo: {res.periodo_servico.inicio} -> {res.periodo_servico.fim}")
    c1, c2 = st.columns(2)
...
with tab2:
    st.subheader("Tempo descontado (contribuicao)")
    if res.periodo_descontado is None:
        st.warning("Sem tempo descontado (inicio do desconto apos o fim de funcoes).")
    else:
        st.info(f"Periodo: {res.periodo_descontado.inicio} -> {res.periodo_descontado.fim}")
    c1, c2 = st.columns(2)
...
with tab3:
    st.subheader("Tempo nao descontado")
    if res.periodo_nao_descontado is None:
        st.success("Nao existe tempo nao descontado neste caso.")
    else:
        st.info(f"Periodo: {res.periodo_nao_descontado.inicio} -> {res.periodo_nao_descontado.fim}")
    c1, c2 = st.columns(2)
...
with tab4:
    st.subheader("Fixacao e pagamento de encargos (LESSOFE)")
    c1, c2, c3 = st.columns(3)
    c1.metric("Valor mensal (7%)", f"{res.valor_mensal:,.2f} Mt")
    c2.metric("Valor diario (mensal/30)", f"{res.valor_diario:,.2f} Mt")
    c3.metric("Encargo total", f"{res.encargo_total:,.2f} Mt")

    st.write("### Prestacoes")
    if res.encargo_total <= 0:
        st.info("Encargo total = 0. Nao ha prestacoes.")
        n_prestacoes = 0
        valor_prest = 0.0
    else:
        limite_terco = (remuneracao_ou_pensao / 3.0) if remuneracao_ou_pensao > 0 else 0.0
        if remuneracao_ou_pensao > 0:
            st.write(f"Limite por prestacao (1/3): {limite_terco:,.2f} Mt")

        min_sugerido = sugerir_min_prestacoes(
            res.encargo_total, remuneracao_ou_pensao, max_prestacoes=60
        )
        if isinstance(min_sugerido, int) and min_sugerido > 0:
            st.info(f"Sugestao (minimo que cumpre 1/3): {min_sugerido} prestacoes.")

        n_prestacoes = st.slider(
            "Escolha o numero de prestacoes",
            1,
            60,
        ...
        st.write(f"Prestacao: {n_prestacoes}x de {valor_prest:,.2f} Mt")

        if remuneracao_ou_pensao > 0 and valor_prest > limite_terco + 1e-9:
            st.error("A prestacao excede 1/3. Aumente o numero de prestacoes.")
    ...
    if st.button("Gerar PDF"):
        if not nome.strip():
            st.error("Informe o nome do funcionario.")
            st.stop()
        ...
        # IA optional demo lines (preferred -> fallback -> deterministic)
        demo_lines = None
        if usar_ia and res.encargo_total > 0:
            payload = {
                ...
                "valor_prestacao_fmt": money_pt(float(valor_prest)) if res.encargo_total > 0 else money_pt(0.0),
            }
            ...
            try:
                ...
                st.success("Demonstracao gerada com IA.")
            except Exception as e:
                st.warning("IA indisponivel (quota/limite). Vou usar demonstracao padrao.")
                st.code(str(e))
                demo_lines = None
...
if gravar:
    if sb is None:
        st.warning("Supabase nao configurado. Defina SUPABASE_URL e SUPABASE_ANON_KEY nos Secrets.")
    else:
    ...
    st.info("Gravacao no Supabase desativada.")
